...
#
# Node groups deliberately live in nodegroups.tf rather than in this module
# call. With Cilium as the sole CNI there is no VPC CNI to make a joining node
# Ready, so Cilium has to be installed in between the control plane and the
# first node. Terraform cannot order a helm_release between a module's cluster
# and that same module's inline eks_managed_node_groups -- adding depends_on
# would create a cycle, since the Helm provider is configured from this
# module's outputs. Splitting compute out makes the ordering explicit:
#
#     control plane -> cilium (wait=false) -> node groups -> addons -> karpenter
#
...
module "eks" {
  ...
  node_security_group_tags = {
    ...
    # Its absence in the previous revision meant Karpenter matched no security
    # group and could never launch an instance.
